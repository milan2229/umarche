  ...
  <div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                  ...
                  <div class="relative">
                    <select name="quantity" class="rounded border appearance-none border-gray-300 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 text-base pl-3 pr-10">
                      @for($i = 1; $i<=$quantity; $i++) <option value="{{ $i }}">{{$i}}</option>
                        <!--1からスタートして1ずつ増えて$quantityと同じになったら停止する  -->
                        @endfor
                    </select>
                  </div>
                </div>
        ...
        <div class="mb-4 text-center">@if ($product->shop->filename !== null)
          <img class="w-40 h-40 rounded-full" src="{{ asset('storage/shop/' . $product->shop->filename )}}">
          @else
          <img src="">
          @endif</div>
    ...
    <div class="modal__overlay" tabindex="-1" data-micromodal-close>
      <div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
        <header class="modal__header">
          <h2 class=class="text-xl text-gray-700" id="modal-1-title">
            {{$product->shop->name}}
          </h2>
